...
class PlayerCar(context: Context,
    ...
    fun update(fps: Long) {
        ...
        if (moving == left && position.left > 0) {          //CAN CHANGE FROM SMOOTH TO FAST/AUTOMATIC
            position.left -= speed / fps                    //So change 'Speed' to a specific pixel jump for position?
        }
